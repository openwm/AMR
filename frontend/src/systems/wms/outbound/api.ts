...
export function useSalesOrders() {
  return useQuery({
    queryKey: KEYS.sos,
    queryFn: async () => {
      const { data } = await api.get<SalesOrder[]>("/api/wms/outbound/sales-orders");
      return data;
    },
  });
}
...
export function useSalesOrder(id: number | undefined) {
  return useQuery({
    ...
    enabled: !!id,
    queryFn: async () => {
      const { data } = await api.get<SalesOrder>(`/api/wms/outbound/sales-orders/${id}`);
      return data;
    },
  });
}
...
export function useShipments(soId?: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Shipment[]>("/api/wms/outbound/shipments", {
...
export function useCreateShipment() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const { data } = await api.post<Shipment>("/api/wms/outbound/shipments", payload);
      return data;
    },
  ...
  return useMutation({
    mutationFn: async (shipmentId: number) => {
      const { data } = await api.post<Shipment>(
        `/api/wms/outbound/shipments/${shipmentId}/ship`,
      );
      return data;
    },
    onSuccess: (data) => {
