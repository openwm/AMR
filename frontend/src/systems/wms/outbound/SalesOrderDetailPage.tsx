...
import { useProducts, useStock } from "@/systems/wms/inventory/api";
...
export default function SalesOrderDetailPage() {
  ...
  if (!so) return <div className="text-muted-foreground">Loadingâ€¦</div>;
        ...
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Customer: {so.customer} Â· Requested: {so.requested_date ?? "â€”"} Â·{" "}
                    ...
                    {form.lines.map((line, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <option value={0}>â€” select location â€”</option>
                            {stocks.map((s) => (
                              <option key={s.id} value={s.location_id}>
                                {s.location_code} ({s.quantity} avail
                                {s.lot_number ? ` Â· lot ${s.lot_number}` : ""})
                              </option>
                            ))}
                          </select>
                          <Input
              ...
              <TableBody>
                {shipments.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>{s.carrier ?? "â€”"}</TableCell>
                    <TableCell>{s.tracking_number ?? "â€”"}</TableCell>
                    <TableCell className="text-muted-foreground">
                      {s.shipped_at ? new Date(s.shipped_at).toLocaleString() : "â€”"}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
