...
import { useLocations, useProducts } from "@/systems/wms/inventory/api";
...
export default function PurchaseOrderDetailPage() {
  ...
  if (!po) return <div className="text-muted-foreground">Loadingâ€¦</div>;
        ...
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Supplier: {po.supplier} Â· Expected:{" "}
              {po.expected_date ?? "â€”"} Â· <Badge variant="outline">{po.status}</Badge>
            </p>
          </div>
