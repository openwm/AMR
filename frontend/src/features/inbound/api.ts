...
export function usePurchaseOrders() {
  return useQuery({
    queryKey: KEYS.pos,
    queryFn: async () => {
      const { data } = await api.get<PurchaseOrder[]>("/api/inbound/purchase-orders");
      return data;
    },
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<PurchaseOrder>(
        `/api/inbound/purchase-orders/${id}`,
      );
      return data;
    },
  });
...
export function useReceipts(poId?: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Receipt[]>("/api/inbound/receipts", {
...
export function useCreateReceipt() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const { data } = await api.post<Receipt>("/api/inbound/receipts", payload);
      return data;
    },
  ...
  return useMutation({
    mutationFn: async (receiptId: number) => {
      const { data } = await api.post<Receipt>(
        `/api/inbound/receipts/${receiptId}/complete`,
      );
      return data;
    },
    onSuccess: (data) => {
