...
from app.systems.wms.outbound import schemas, service
...
@router.patch("/sales-orders/{so_id}/lines/{line_id}", response_model=schemas.SOLineRead)
def update_line_picked(
    so_id: int,
    line_id: int,
    payload: schemas.SOLinePickedUpdate,
    db: Session = Depends(get_db),
):
    """WCS callback: record picked quantity on a SO line and auto-ship when all lines are done."""
    try:
        line = service.record_line_picked(db, so_id, line_id, payload.picked_qty)
        db.commit()
        db.refresh(line)
        return line
